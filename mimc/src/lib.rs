...
impl Mimc {
    pub fn new(n_rounds: usize) -> Mimc {
        Mimc {
...
mod tests {
    ...
    fn test_mimc() {
        ...
        let mimc = Mimc::new(91);
        let h1 = mimc.hash(&b1, &b2);
        assert_eq!(
            h1.to_string(),
        ...
        arr.push(b2.clone());
        arr.push(b3.clone());
        let h1 = mimc.multi_hash(arr, &Fr::zero());
        assert_eq!(
            h1.to_string(),
        ...
        let h2 = mimc.multi_hash(big_arr1, &Fr::zero());
        assert_eq!(
            h2.to_string(),
