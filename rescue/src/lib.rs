...
mod rescue_prime_tests {
    use super::*;
    use rand::thread_rng;
    use rescue_prime_instance_bn256::RESCUE_PRIME_BN_PARAMS;
    use ff::{from_hex, Field, PrimeField};
    // use rescue_prime_params::Fr;
    ...
    fn kats() {
        let rescue_prime = RescuePrime::new(&RESCUE_PRIME_BN_PARAMS);
        let input: Vec<Scalar> = vec![Scalar::zero(), Scalar::one(), Scalar::from_repr(Scalar::Repr::from(2)).unwrap()];
